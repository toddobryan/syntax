 ...
 * Ported from Java version by Todd O'Bryan <toddobryan@gmail.com>
 */
 ...
 */
const DART_TOKENIZER_TEMPLATE = fs.readFileSync(
  `${__dirname}/templates/tokenizer.template.dart`,
  'utf-8'
);
...
const DEFAULT_PARSER_EVENTS_CLASS = `
  class ParserEvents {
    ...
    static void onParseBegin(String _string) {
      // Parsing is started.
    }

    static void onParseEnd(Object _result) {
...
/**
 * The trait is used by parser generators (LL/LR) for Dart.
 */
const DartParserGeneratorTrait = {
   ...
   *
   * In Java we represent a table as a List<Map<int, String>> mTable.
   *
   * Example:
   *
   *   mTable = [
  ...
  _buildTable(table) {
    const entries = Object.keys(table).map(state => {
      return '<int, String>{' +
        this._toDartHashMap(table[state], 'Integer', 'String') + '};';
    });
  ...
  generateTokensTable() {
    this.writeData(
      'TOKENS',
      this._toDartHashMap(this._tokens, 'String', 'Integer'),
    );
  },
  ...
  generateProductionsData() {
    return this.generateRawProductionsData()
      .map(data => {
        // Remove the semantic action handler, since in Dart
  ...
  generateBuiltInTokenizer() {
    this.writeData('TOKENIZER', DART_TOKENIZER_TEMPLATE);
  },
  ...
  generateLexRulesByStartConditions() {
    ...
    this.writeData(
      ...
      `${this._toDartHashMap(result, 'String', 'List<int>')}`,
    );
  },
  ...
  /**
   * Converts JS object to Dart HashMap representation.
   */
  _toDartHashMap(object, keyType, valueType) {
    let result = [];
    for (let k in object) {
      let value = object[k];
      let key = k.replace(/"/g, '\\"');
      result.push(
        `${this._hashKey(key, keyType)}: ${this._hashValue(value, valueType)});`
      );
    }
    return `{\n    ${result.join(',\n    ')}, \n  }`;
  },
  ...
  /**
   * Dart-specific lex rules handler declarations.
   */
  generateLexHandlers() {
  ...
  /**
   * Dart-specific handler declarations.
   */
  ...
  /**
   * Productions array in Dart format.
   */
  generateProductions() {
    this.writeData(
      'PRODUCTIONS',
